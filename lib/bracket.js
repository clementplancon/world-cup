...
const MATCH_CITIES = {
  ...
  97: "Los Angeles", 98: "Kansas City", 99: "Boston", 100: "Miami",
...
function placeholderMatch(matchNum) {
  if (MATCH_TEAMS[matchNum]) {
    ...
    return {
      ...
      city: MATCH_CITIES[matchNum] || null,
    };
  }
  return { home: null, away: null, score: { home: null, away: null }, penalties: null, status: "scheduled", live: null, winner: null, date: MATCH_DATES[matchNum] || null, city: MATCH_CITIES[matchNum] || null };
}
...
function matchCity(mt, matchNum) {
  const venue = mt && mt.venue && typeof mt.venue === "object" ? mt.venue : null;
  const location = venue && venue.location && typeof venue.location === "object" ? venue.location : null;
  return firstString(
    mt && mt.city,
    mt && mt.venueCity,
    mt && mt.hostCity,
...
function toMatchObject(mt, matchNum) {
  ...
  return {
    home: homeCode ? { code: homeCode, name: mt.homeTeam.shortName || mt.homeTeam.name || CODE_TO_NAME[homeCode] } : null,
    away: awayCode ? { code: awayCode, name: mt.awayTeam.shortName || mt.awayTeam.name || CODE_TO_NAME[awayCode] } : null,
...
function fillFromChildren(match, matchNum, matchResults) {
  ...
  const home = (match.home && match.home.code) ? match.home : winnerOf(matchResults[childA]);
  const away = (match.away && match.away.code) ? match.away : winnerOf(matchResults[childB]);

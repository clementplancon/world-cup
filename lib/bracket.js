...
// How long a kickoff slot is considered "active": from a little before the
// scheduled start until well after it could plausibly finish (90' + stoppage
// + potential extra time + penalties + post-match reporting lag from the API).
const MATCH_WINDOW_BEFORE_MS = 15 * 60 * 1000; // 15 min before kickoff
const MATCH_WINDOW_AFTER_MS = 3.5 * 60 * 60 * 1000; // 3h30 after kickoff

// Rate-limit optimisation (football-data.org allows 10 req/min): there is no
// point polling the API when no knockout match could possibly be in progress.
// Returns true when `now` falls inside any fixed match window from MATCH_DATES.
function isInsideAMatchWindow(now = new Date()) {
  const t = now.getTime();
  return Object.values(MATCH_DATES).some((iso) => {
    const start = new Date(iso).getTime();
    return t >= start - MATCH_WINDOW_BEFORE_MS && t <= start + MATCH_WINDOW_AFTER_MS;
  });
}

// Safety net on top of the fixed windows above. A match can outlast its window
// (extra time + penalties) or, more commonly, football-data.org's free tier can
// publish the FINISHED status/score only after the window has already closed.
// When that happens, the last poll inside the window still saw the match as
// live (or scheduled-but-started), the poller then goes quiet, and the bracket
// is frozen with that match shown as "kicking off"/live and no final score
// until the *next* match window opens hours later. To avoid that, we keep
// polling while the current bracket still holds a match that has already
// kicked off but isn't `final` yet — capped by a generous grace period so a
// fixture the API never resolves can't keep us polling indefinitely.
const MATCH_RESOLUTION_GRACE_MS = 6 * 60 * 60 * 1000; // 6h after kickoff

function hasUnresolvedStartedMatch(bracket, now = new Date()) {
  if (!bracket || !Array.isArray(bracket.rounds)) return false;
  const t = now.getTime();
  return bracket.rounds.some((round) =>
    Array.isArray(round) &&
    round.some((m) => {
      if (!m || m.status === "final") return false;
      // A live match must always be followed until it turns final.
      if (m.status === "live") return true;
      // A scheduled match whose kickoff has passed but that the API hasn't
      // updated yet: keep polling within the grace period so we catch it going
      // live/final instead of stranding it.
      const kickoff = m.date ? new Date(m.date).getTime() : NaN;
      if (!Number.isFinite(kickoff)) return false;
      return t >= kickoff && t <= kickoff + MATCH_RESOLUTION_GRACE_MS;
    })
  );
}
...
//
//  - Skips the API call entirely when we're outside every match window AND we
//    already have data to keep serving (rate-limit optimisation). Returns null
//    so the caller keeps `previousData` untouched.
//  - On the very first run (previousData == null) it always fetches, so a fresh
//    process/restart bootstraps a bracket even if it starts outside a window.
...
//
// Returns `{ updated, rounds }` on success, or `null` when there's nothing to do.
async function fetchAndBuildBracket({ apiKey, previousData = null } = {}) {
  if (!apiKey) {
    throw new Error("Missing FOOTBALL_DATA_API_KEY");
  }

  if (previousData && !isInsideAMatchWindow() && !hasUnresolvedStartedMatch(previousData)) {
    // Outside any match window, and every already-started match in the current
    // bracket is final (nothing left to resolve): skip the API call to stay
  ...
  return { updated: new Date().toISOString(), rounds };
}
...
module.exports = {
  ...
  toMatchObject,
  buildRounds,
  isInsideAMatchWindow,
  hasUnresolvedStartedMatch,
